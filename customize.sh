#
# Customization script for the Magisk Module with clang20 and NDK r29-beta1 
#
# History
#   14.04.2025 /bs
#     initial release
...
LogMsg "Installing clang20 and the NDK r29-beta1 files for Linux ..."
...
for CUR_FILE in ${SAMPLE_PROGRAMS} ; do
  if [ -r "${MODPATH}/${CUR_FILE}" ] ; then
    if [ -r "/data/local/tmp/${CUR_FILE}" ] ; then
      ...
      cp "${MODPATH}/${CUR_FILE}" "/data/local/tmp/${CUR_FILE}"
    fi
  else
    LogWarning "The file \"${MODPATH}/${CUR_FILE}\" does not exist"
  fi
done
...
to init the environment for the clang20
